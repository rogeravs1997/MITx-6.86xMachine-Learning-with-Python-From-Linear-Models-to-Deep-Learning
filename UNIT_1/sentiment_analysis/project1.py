...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    z=labels*(theta.dot(feature_matrix.T)+theta_0)
    losses=[0 if i >=1 else 1-i for i in z ]
...
def perceptron_single_step_update(
    ...
    completed.
    """
    z=label*(current_theta.dot(feature_vector.T)+current_theta_0)
    if z <= 0:
        updated_theta=current_theta+(label*feature_vector)
        update_theta_0=current_theta_0+label
    else:
        updated_theta=current_theta
        update_theta_0=current_theta_0
    return (updated_theta,update_theta_0)
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            updated_thetas = perceptron_single_step_update(feature_vector=feature_matrix[i,:],
                                          label=labels[i],
                                          current_theta=theta,
                                          current_theta_0=theta_0)
            theta=updated_thetas[0]
            theta_0=updated_thetas[1]
...
def average_perceptron(feature_matrix, labels, T):
    ...
    return (np.mean(thetas,axis=0),np.mean(thetas_0))
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        ...
        for i in get_order(feature_matrix.shape[0]):
            ...
            updated_thetas =  pegasos_single_step_update(
...
def classify(feature_matrix, theta, theta_0):
    ...
    return np.array([1 if x==True else -1 for x in predictions])
